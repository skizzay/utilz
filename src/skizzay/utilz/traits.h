...
template<class T, template<class ...> class Template>
constexpr bool inherits_from_template_v  = inherits_from_template<T, Template>::value;
...
template<class T>
constexpr auto to_reference(T &t) noexcept -> dereference_t<std::remove_reference_t<T>> {
   return *t;
}
...
>;
template<class T, class U=T>
constexpr bool equivalent_v = equivalent<T, U>::value;
...
template<class T>
constexpr bool is_dto_v = is_dto<T>::value;
